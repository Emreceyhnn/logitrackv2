...
export function useDriverMutations() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => handleError("Failed to create driver", error),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => handleError("Failed to update driver", error),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteDriver(id),
    onSuccess: () => handleSuccess("Driver deleted successfully"),
    onError: (error) => handleError("Failed to delete driver", error),
  });
  ...
  const statusMutation = useMutation({
    ...
    onError: (error) => handleError("Failed to update driver status", error),
  });
  ...
  const assignMutation = useMutation({
    ...
    onError: (error) => handleError("Failed to assign vehicle", error),
  });
  ...
  const unassignMutation = useMutation({
    ...
    onError: (error) => handleError("Failed to unassign vehicle", error),
  });
