...
export const StatusChip = ({ status }: { status: string }) => {
    ...
    const getStatusColor = (status: string) => {
        ...
        // Success - Green
        if (["AVAILABLE", "COMPLETED", "VALID", "ON_DUTY", "DELIVERED", "SUCCESS"].includes(normalizedStatus)) {
            return { bgColor: "#d3f9e3", color: "#009966" };
        }

        // Info - Blue
        if (["ON_TRIP", "IN_PROGRESS", "IN_TRANSIT", "PICKED_UP", "IN_SERVICE"].includes(normalizedStatus)) {
            return { bgColor: "#e3f2fd", color: "#0288d1" };
        }

        // Warning - Yellow/Orange
        if (["DUE_SOON", "WARNING", "LOW", "MEDIUM", "PLANNED"].includes(normalizedStatus)) {
            return { bgColor: "#fff3cd", color: "#f57c00" };
        }

        // Error - Red
        if (["ERROR", "EXPIRED", "HIGH", "FAILED", "CRITITCAL"].includes(normalizedStatus)) {
