...
const CustomerDetailDialog = ({ open, onClose, customerId }: CustomerDetailDialogParams) => {
                ...
                <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, minHeight: 400 }}>
                    ...
                    <Box sx={{
                        ...
                        <Stack spacing={4}>
                            {/* Primary Contact */}
                            <Box>
                                <Typography variant="overline" color="text.secondary" fontWeight={700} letterSpacing={1.2} display="block" mb={2}>
                                    Primary Contact
                                </Typography>
                                {customer.contacts[0] ? (
                                    <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, bgcolor: 'background.paper' }}>
                                        <Stack spacing={1.5}>
                                            ...
                                            <Stack spacing={1}>
                                                <Stack direction="row" spacing={1.5} alignItems="center">
                                                    <PhoneIcon fontSize="small" color="action" sx={{ fontSize: '1rem' }} />
                                                    <Typography variant="body2">{customer.contacts[0].phone}</Typography>
                                                </Stack>
                                                <Stack direction="row" spacing={1.5} alignItems="center">
                                                    <EmailIcon fontSize="small" color="action" sx={{ fontSize: '1rem' }} />
                                                    <Typography variant="body2" sx={{ wordBreak: 'break-all' }}>{customer.contacts[0].email}</Typography>
                                                </Stack>
                                            </Stack>
                                        </Stack>
                                    </Paper>
                                    ...
                                    <Stack direction="row" spacing={1.5} alignItems="flex-start">
                                        ...
                                        <Box>
                                            ...
                                            <Typography variant="body2" color="text.secondary">
                                                {customer.billingAddress.district}, {customer.billingAddress.city}
                                            </Typography>
                                            <Typography variant="body2" color="text.secondary">
                                                {customer.billingAddress.country.toUpperCase()}, {customer.billingAddress.postalCode}
                                            </Typography>
                                        </Box>
                                    </Stack>
                    ...
                    <Box sx={{ width: { xs: '100%', md: '65%' }, p: 3 }}>
                        ...
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ mb: 4 }}>
                                ...
                                <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                                    <Stack spacing={1}>
                                        <Stack direction="row" justifyContent="space-between">
                                            <Typography variant="caption" fontWeight={600} color="text.secondary">TOTAL ORDERS</Typography>
                            ...
                            <Box sx={{ flex: 1 }}>
                                <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                                    <Stack spacing={1}>
                                        <Stack direction="row" justifyContent="space-between">
                                            <Typography variant="caption" fontWeight={600} color="text.secondary">ACTIVE</Typography>
                                            <AccessTimeIcon fontSize="small" color="warning" />
                                        </Stack>
                                        <Typography variant="h5" fontWeight={700}>{activeOrders}</Typography>
                                    </Stack>
                                </Paper>
                            </Box>
                            <Box sx={{ flex: 1 }}>
                                <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                                    <Stack spacing={1}>
                                        <Stack direction="row" justifyContent="space-between">
                                            <Typography variant="caption" fontWeight={600} color="text.secondary">ON-TIME RATE</Typography>
                                            <VerifiedIcon fontSize="small" color="success" />
                                        </Stack>
                                        <Typography variant="h5" fontWeight={700}>{customer.sla.onTimeTargetPct}%</Typography>
                                    </Stack>
                                </Paper>
                            </Box>
                        </Stack>
                        ...
                        <Box>
                            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                                ...
                                <Chip label={`${customer.deliverySites.length} Locations`} size="small" sx={{ height: 20, fontSize: '0.65rem', fontWeight: 600 }} />
                            </Stack>

                            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: 2 }}>
                                {customer.deliverySites.map((site) => (
                                    <Paper key={site.id} variant="outlined" sx={{ p: 2, borderRadius: 2, '&:hover': { borderColor: 'primary.main', bgcolor: alpha(theme.palette.primary.main, 0.02) }, transition: 'all 0.2s', cursor: 'pointer' }}>
                                        <Stack direction="row" spacing={2} alignItems="center">
                                            <Avatar variant="rounded" sx={{ bgcolor: alpha(theme.palette.grey[500], 0.1), color: 'text.primary' }}>
                                                <BusinessIcon />
                                            </Avatar>
                                            <Box>
