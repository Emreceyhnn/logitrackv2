...
mock.module("@/app/components/chips/statusChips", {
  namedExports: {
    StatusChip: ({ status }: unknown) => <span data-testid="status-chip">{status}</span>,
  },
});
...
describe("DriverTable RTL Component", () => {
  let DriverTable: React.ElementType;
  ...
  describe("DriverTable() bileşeni", () => {
    ...
    it("should_FormatColumnsCorrectly", async () => {
      ...
      // 1. name
      const nameCol = columns.find((c: unknown) => c.key === "name");
