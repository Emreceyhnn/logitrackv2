...
const VehicleKpiCard = () => {
    ...
    const openIssues = fleet.reduce((acc, v) => acc + (v.maintenance?.openIssues?.length || 0), 0);
    const docsDueSoon = fleet.reduce((acc, v) => acc + (v.documents?.filter(d => new Date(d.expiresOn) <= new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)).length || 0), 0);
