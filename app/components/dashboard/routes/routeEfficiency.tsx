...
const RouteEfficiency = () => {
    ...
    const avgFuelConsumption = mockData.vehicles.reduce(
        (sum, v) => sum + v.fuel.consumptionLPer100Km,
        0
    ) / mockData.vehicles.length


    const vehicleLength = mockData.vehicles.map(i => i).length
    const vehicleUtilization = mockData.vehicles.filter(i => i.status === "ON_TRIP").length / vehicleLength * 100;
    ...
    return (
        <CustomCard sx={{ display: "flex", flexDirection: "column", gap: 3, flexGrow: 1 }}>
            ...
            <Stack spacing={1}>
                ...
                <LinearProgress variant="determinate" value={vehicleUtilization} sx={{
                    bgcolor: "background.dashboardBg",
                    borderRadius: "8px",
                    "& .MuiLinearProgress-bar": {
                        borderRadius: "8px",
