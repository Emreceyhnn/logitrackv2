...
const setType: Record<ActionType, ReactNode> = {
  SHIPMENT_DELAY: (
    ...
    >
      <LocalShippingIcon
        sx={{ color: "#fff", width: 18, height: 18, p: 0, m: 0 }}
      />
    </Box>
  ),
  vehicle: (
    ...
    >
      <DirectionsCarIcon sx={{ color: "#fff", fontSize: 18 }} />
    </Box>
  ),
  driver: (
    <Box
  ...
  DOCUMENT_DUE: (
    ...
    >
      <SummarizeIcon sx={{ color: "#fff", fontSize: 18 }} />
    </Box>
  ),
  warehouse: (
    <Box
...
const ActionRequiredCard = ({ alerts = [] }: ActionRequiredCardProps) => {
  return (
    <CustomCard sx={{ padding: "0 0 6px 0" }}>
      <Stack minWidth={400}>
        <Typography
          sx={{ fontSize: 18, fontWeight: 600, bgcolor: "#136b86", padding: 1 }}
        >
          Action Required
        </Typography>
